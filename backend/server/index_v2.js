...
function stepSimulationV2() {
  for (const [clientId, antList] of Object.entries(ANTS)) {
...
setInterval(stepSimulationV2, TICK_INTERVAL);
  ...
  ws.on("message", (msg) => {
    try {
      ...
      if (type === "placeAnt") {
        ...
        ANTS[clientId].push({ ...payload, color, dir: 0 });
      ...
      if (type === "setRules") {
        ...
        if (ANTS[clientId]) {
          for (const ant of ANTS[clientId]) {
            ant.rules = filtered;
          }
        } else {
          ANTS[clientId] = [
            { x: 50, y: 50, dir: 0, color, rules: filtered }
          ];
        }
